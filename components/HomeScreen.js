  ...
  <ScreenLayout>
    <ScrollView contentContainerStyle={tw`flex-grow items-center bg-purple-50 p-5`}>
      ...
      <View style={tw`items-center mt-12 mb-4`}>
        <View
          style={tw`w-24 h-24 bg-white rounded-full border-4 border-purple-400 mb-2`}
        >
          <Image
      ...
      {Object.keys(categories).map((cat) => (
        <View
          key={cat}
          style={tw`flex-row items-center w-5/6 mb-4`}
        >
          <TouchableOpacity
            style={tw`bg-purple-500 py-4 px-4 flex-1 rounded-lg`}
            onPress={() => navigation.navigate("Game", { category: cat })}
            activeOpacity={0.8}
          >
